...
int main(){
    ...
    Joypad joypad;
    joypad.write(0xCF);  // Select D-Pad(bits 4-5 = 0)
    ...
    joypad.write(0xCF);  // Select D-Pad
    ...
    joypad2.write(0xDF);  // Select buttons(bit 5 = 1, bit 4 = 1)
    uint8_t button_state = joypad2.read();
    std::cout << "After pressing A(button select): 0x" << std::hex << (int)button_state << std::dec;
    // When bit 4 = 1 (buttons NOT selected), should read 0x0F
    if((button_state & 0x0F) == 0x0F){
        std::cout << " ✓" << std::endl;
    } else {
    ...
    joypad2.write(0xDF);  // Select only D-Pad(bit 5=0, bit 4=1)
    uint8_t dpad_only = joypad2.read();
    joypad2.write(0xEF);  // Select only buttons(bit 5=1, bit 4=0)
